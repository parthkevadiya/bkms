...
import 'manage_kst_quiz_score_fillters.dart';

class ManageKSTQuizScores extends StatefulWidget {
  static String id = "manageKSTQuizScores";
  static String path = "${ManageKSTReport.path}/$id";

  const ManageKSTQuizScores({Key? key}) : super(key: key);

  @override
  _ManageKSTQuizScoresState createState() => _ManageKSTQuizScoresState();
}

class _ManageKSTQuizScoresState extends State<ManageKSTQuizScores> {
  //FOR PAGINATION
  int _totalKSTQuizScores = 0;
  int _currentPage = 1;
  ...
  ManageKSTQuizScoreModel? _manageKSTQuizModel;
  List<ManageKSTQuizListDataModel?>? _manageKSTQuizItems = [];
  List<ManageKSTQuizListDataModel?>? _searchItems = [];

  List<ManageKSTQuizListDataModel?> _allRecords = [];
  ...
  void onRefresh(
    ...
    //FOR GET REPORTS
    BlocProvider.of<ManageKSTReportBloc>(context).add(ManageKSTReportEvent.manageKSTQuizScore(
      context,
      id,
      isForEditMode(),
      _strSelectedWing,
  ...
  void initState() {
    ...
    //FOR GET MANAGE KST QUIZ SCORES
    BlocProvider.of<ManageKSTReportBloc>(context).add(ManageKSTReportEvent.manageKSTQuizScore(
      context,
      ...
      listeners: [
        BlocListener<KSTDataUpdateBloc, KSTDataUpdateState>(
          listener: (context, state) {
            state.maybeWhen(
                saveKSTQuizScore: (saveKSTQuizScoreModel, l) {
                  if (l == LoadingStatus.Done) {
                    _hideLoadingIndicator();
                    if (saveKSTQuizScoreModel != null && !saveKSTQuizScoreModel.has_error!) {
                      if (_search.text.isEmpty) {
                        _scrollController.initialRefresh;
                        //      _pagingController.refresh();
                      }
                    } else {
                      _manageKSTQuizItems![_currentIndex]!.dynamic_field![_currentParentIndex]!.selected =
        ...
        BlocListener<ManageKSTReportBloc, ManageKSTReportState>(
          listener: (context, state) {
            state.maybeWhen(
                manageKSTQuizScore: (manageKSTQuizScoreModel, l) async {
                  if (l == LoadingStatus.Done) {
                    _manageKSTQuizItems!.clear();
                    _hideLoadingIndicator();
                    _totalKSTQuizScores = 0;
                    if (manageKSTQuizScoreModel != null && manageKSTQuizScoreModel.kst_quiz_score_result != null) {
                      _manageKSTQuizModel = manageKSTQuizScoreModel;
                      _totalKSTQuizScores = Utils().nullSafeInt(_manageKSTQuizModel!.kst_quiz_score_result!.total);

                      if (_manageKSTQuizModel!.kst_quiz_score_result!.data!.length == _allRecords.length) {
                        setState(() {
                          loading = false;
                        });
                      } else {
                      ...
                      final allRecords = _manageKSTQuizModel!.kst_quiz_score_result!.data;
                      _allRecords = allRecords!
                          .map(
                            (e) => ManageKSTQuizListDataModel.fromJson(
                              jsonDecode(
                                jsonEncode(e),
                              ),
                            ),
        ...
        BlocListener<ParamsBloc, ParamsState>(
          listener: (context, state) {
            if (state is ManageKSTQuizScoreFiltersState) {
              ...
              BlocProvider.of<ParamsBloc>(context).add(ManageKSTQuizScoreFiltersEvent(
                _strSelectedWing,
                _strSelectedCenter,
                _strSelectedRegion,
                _strSearchUserId,
        ...
        Column(
          ...
          children: [
                                      ...
                                      : InkWell(
                                          onTap: () {
                                            ...
                                            BlocProvider.of<ManageKSTReportBloc>(context)
                                                .add(ManageKSTReportEvent.manageKSTQuizScore(
                                              context,
                                              Utils().nullSafeString(_manageKSTReportListDataModel!.id),
                                              isForEditMode(),
                                              _strSelectedWing,
            ...
            Flexible(
              child: Stack(
                children: [
                  if (_manageKSTQuizItems!.isEmpty)
                    Center(
                      child: Container(
                  ...
                  SmartRefresher(
                    ...
                    child: ListView.builder(
                        ...
                        shrinkWrap: true,
                        itemCount: (_manageKSTQuizItems?.length ?? 0) + 1,
                        itemBuilder: (context, index) => (_manageKSTQuizItems!.length == index)
                            ? SizedBox(
                                height: 50,
                              )
                            : Column(
                                  ...
                                  Padding(
                                    ...
                                    child: ExpansionTile(
                                      ...
                                      onExpansionChanged: (val) {
                                        setState(
                                          () {
                                            _manageKSTQuizItems![index]!.isExpand = val;
                                          },
                                        );
                                      },
                                      // trailing: ((isForSearch()
                                      //                     ? _searchItems![
                                      //                         index]!
                                      //                     : _manageKSTQuizItems![
                                      //                         index]!)
                                      ...
                                      title: Column(
                                        ...
                                        children: [
                                          Text(
                                            Utils().nullSafeString(_manageKSTQuizItems![index]!.full_name) +
                                                " (" +
                                                Utils().nullSafeString(_manageKSTQuizItems![index]!.user_id) +
                                                ")",
                                            style: TextStyle(
                                          ...
                                          Padding(
                                            ...
                                            child: Text(Utils().nullSafeString(
                                                Utils().nullSafeString(_manageKSTQuizItems![index]!.RegionName) +
                                                    " | " +
                                                    Utils().nullSafeString(_manageKSTQuizItems![index]!.CenterName) +
                                                    " | " +
                                                    Utils().nullSafeString(_manageKSTQuizItems![index]!.group_name))),
                                          ),
                                        ],
                                      ),
                                      children: [
                                          ...
                                          children: [
                                            Padding(
                                              ...
                                              child: Text(Utils().nullSafeString("Grade: " +
                                                  Utils().nullSafeString(_manageKSTQuizItems![index]!.grade))),
                                            ),
                                          ],
                                        ),
                                        ListView(
                                          ...
                                          children: [
                                            Column(
                                              ...
                                              children: List.generate(
                                                  _manageKSTQuizItems![index]!.dynamic_field!.length, (parentIndex) {
                                                var currentItem =
                                                    _manageKSTQuizItems![index]!.dynamic_field![parentIndex]!;
                                                            ...
                                                            child: DropdownButtonHideUnderline(
                                                              child: DropdownButton(
                                                                ...
                                                                onChanged: (newValue) async {
                                                                  ...
                                                                  setState(() {
                                                                    var selectedDropdownItem =
                                                                        newValue as MKSTQDOptionsModel;
                                                                    _currentIndex = index;
                                                                    _currentParentIndex = parentIndex;
                                                                    _peviousSelectedOption =
                                                                        _manageKSTQuizItems![index]!
                                                                    ...
                                                                    var item = _manageKSTQuizItems![index]!;

                                                                    //FOR SAVE QUIZ SCORE
                                                                    _showLoadingIndicator();
                                                                    var params = SaveKSTQuizScoreRequestModel(
                                                                      ...
                                                                      Utils().nullSafeString(_manageKSTQuizModel!
                                                                          .kst_quiz_score_filter!
                                                                    ...
                                                                    BlocProvider.of<KSTDataUpdateBloc>(context)
                                                                        .add(KSTDataUpdateEvent.saveKSTQuizScore(
                                                                      context,
                                                                      params,
                                                                    ));
                                                                  });
                    ...
                    child: Container(
                      ...
                      child: Padding(
                        ...
                        child: Row(
                          children: [
                            Text(
                              _manageKSTQuizItems!.isEmpty
                                  ? "Showing no results based on filters. "
                                  : ((_search.text.isEmpty)
                                      ? "Showing all ${_totalKSTQuizScores.toString()}  result. "
                                      : "Showing all ${_manageKSTQuizItems!.length}  result. "),
                              style: TextStyle(
                                fontSize: 12.6,
                                    ...
                                    onTap: () {
                                      ...
                                      (_search.text.isEmpty)
                                          ...
                                          : BlocProvider.of<ManageKSTReportBloc>(context).add(
                                              ManageKSTReportEvent.manageKSTQuizScore(
                                                context,
                                                Utils().nullSafeString(_manageKSTReportListDataModel!.id),
                                                isForEditMode(),
                                                _strSelectedWing,
...
//endregion
  ...
  openFilter() {
    showMaterialModalBottomSheet(
      ...
      expand: false,
      context: context,
      builder: (context) => ManageKSTQuizScoreFilter(
        onSelectFilter: (
          bool callBackFilter,
          String selectedWing,
