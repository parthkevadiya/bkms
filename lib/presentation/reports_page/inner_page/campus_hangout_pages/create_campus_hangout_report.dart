...
class CreateCampusHangoutReport extends StatefulWidget {
  static String id = "createCampusHangoutReport";
  static String path = "${CampusHangout.path}/$id";

  @override
  _CreateCampusHangoutReportState createState() => _CreateCampusHangoutReportState();
}

class _CreateCampusHangoutReportState extends State<CreateCampusHangoutReport> with TickerProviderStateMixin {
  List<CampusRegionItems?>? _regionItems;
  CampusRegionItems? _selectedRegion;

  List<HCampusModel?>? _campusItems = [];
  HCampusModel? _selectedCampus;

  List<HangoutTypesModel?>? _wingItems = [];
  HangoutTypesModel? _selectedWing;
  ...
  void initState() {
    ...
    if (paramsBloc is CreateCampusHangoutReportState) {
      _regionItems = paramsBloc.hangoutRegions;
    }

    //FOR ADD DATA
    BlocProvider.of<ParamsBloc>(context).add(CreateCampusHangoutReportEvent(_regionItems));
  }
      ...
      listeners: [
            ...
            state.maybeWhen(
                ...
                campusWiseHangoutTypes: (campusWiseHangoutTypes) {
                  if (campusWiseHangoutTypes != null) {
                    _wingItems = campusWiseHangoutTypes.hangout_types!;
                  }
                },
                orElse: () {});
          },
        ...
        BlocListener<CampusHangoutBloc, CampusHangoutState>(
          listener: (context, state) {
            ...
            state.maybeWhen(
                createCampusHangoutReport: (createCampusHangoutReportModel) {
                  if (!createCampusHangoutReportModel!.has_error!) {
                    //FOR UPDATE CAMPUS HANGOUT REPORT LIST
                    BlocProvider.of<ParamsBloc>(context).add(CampusHangoutReportRefreshEvent(true));
                                ...
                                child: ListView(
                                  ...
                                  children: [
                                    ...
                                    Container(
                                      ...
                                      child: DropdownButton<CampusRegionItems>(
                                        ...
                                        onChanged: (value) {
                                          setState(() {
                                            ...
                                            BlocProvider.of<CampusHangOutDataUpdateBloc>(context).add(
                                                CampusHangOutDataUpdateEvent.hRegionWiseCampus(context,
                                                    Utils().nullSafeString(_selectedRegion!.region_id.toString())));
                                          });
                                        },
                                        ...
                                        ElevatedButton(
                                          ...
                                          onPressed: () async {
                                            if (_selectedWing != null) {
                                              //FOR CREATE CAMPUS HANGOUT REPORT
                                              _showLoadingIndicator();
                                              BlocProvider.of<CampusHangoutBloc>(context).add(
                                                  CampusHangoutEvent.createCampusHangoutReport(
                                                      context,
                                                      Utils().nullSafeString(_selectedRegion!.region_id.toString()),
                                                      Utils().nullSafeString(_selectedCampus!.id.toString()),
                                                      Utils().nullSafeString(_selectedWing!.id.toString())));
                                            }
                                          },
                                          child: Padding(
